...
@Injectable()
export class Utils {
    public static get JSONHeader() {
        return new HttpHeaders().set('Content-Type', 'application/json');
    }
    public static get FormURLEncodedHeader() {
    ...
    public static isPhoneNumber(phone: string): boolean {
        return phone ?
            phone.match(/^[+]*[(]{0,1}[0-9]{1,3}[)]{0,1}[-\s\./0-9]*$/g) !== null : false;
    }
    ...
    public static enumToDropdown(e): IDropdownOption[] {
        ...
        for (const item in e) {
            // discard numeric keys of the enum
            if (e.hasOwnProperty(item) && !Utils.isNumber(item)) {
    ...
    public isDev() {
        return this.config.environment === Environments.dev ||
            this.config.environment === Environments.browser;
    }
    public isProd() {
