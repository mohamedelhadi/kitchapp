...
export class Branches extends BasePage {
        ...
        this.data.getRestaurantBranches(this.restaurant.id)
            ...
            .subscribe(branches => {
                branches.forEach(branch => {
                    branch.rate.nUserGaveRate1 =
                        Math.ceil(branch.rate.ratesCounts[0] * 10 / branch.rate.usersCount);
                    branch.rate.nUserGaveRate2 =
                        Math.ceil(branch.rate.ratesCounts[1] * 10 / branch.rate.usersCount);
                    branch.rate.nUserGaveRate3 =
                        Math.ceil(branch.rate.ratesCounts[2] * 10 / branch.rate.usersCount);
                    branch.rate.nUserGaveRate4 =
                        Math.ceil(branch.rate.ratesCounts[3] * 10 / branch.rate.usersCount);
                    branch.rate.nUserGaveRate5 =
                        Math.ceil(branch.rate.ratesCounts[4] * 10 / branch.rate.usersCount);
                });
    ...
    public onBackButton() {
        // this.navCtrl.parent.parent.pop(); switch to first tab because it feels natural
