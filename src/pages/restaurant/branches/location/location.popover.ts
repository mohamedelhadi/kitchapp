...
export class LocationPopover extends BasePopover implements OnInit {
    ...
    constructor(
        ...
        this.locationUrl = this.sanitizer.bypassSecurityTrustUrl(
            'geo:' + this.branch.location.latitude + ',' + this.branch.location.longitude
        );
    }
    public ngOnInit(): void {
        ...
        if (Utils.isOnline()) {
            ...
            // manually hiding as ngIf causes @ViewChild("map") to fail
    ...
    private loadMap() {
        // failed to load google maps script due to network, ad-block, ..etc.
        if (google === undefined) {
            return;
        }
        const coordinates = {
            lat: this.branch.location.latitude, lng: this.branch.location.longitude
        };
