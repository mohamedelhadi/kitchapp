...
export class RestaurantsPopover extends BasePopover implements OnInit {
    ...
    public ngOnInit(): void {
        super.ngOnInit();
        const searchSettings$: BehaviorSubject<IRestaurantsSearchSettings> =
            this.params.data.settings;
        searchSettings$
            .takeUntil(this.viewCtrl.willUnload)
            .subscribe(settings => this.searchSettings = settings);
    ...
    public onChange() {
        (this.params.data.settings as BehaviorSubject<IRestaurantsSearchSettings>)
            .next(this.searchSettings);
    }
    public close() {
